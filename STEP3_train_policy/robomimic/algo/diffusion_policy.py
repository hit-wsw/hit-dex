...
class DiffusionPolicyUNetDex(PolicyAlgo):
    def _create_networks(self):
        ...
        if self.obs_shapes["robot0_eef_pos"][0] == 3: # 单手
            self.arm_split = 3
    ...
    def train_on_batch(self, batch, epoch, validate=False):
        ...
        with TorchUtils.maybe_no_grad(no_grad=validate):
            info = super(DiffusionPolicyUNetDex, self).train_on_batch(batch, epoch, validate=validate)
            actions = batch['actions']

            if self.use_handnoise and "robot0_eef_hand" in self.obs_shapes.keys():
                handnoise = torch.randn_like(batch["obs"]["robot0_eef_hand"]) * 0.03
                batch["obs"]["robot0_eef_hand"] += handnoise
            ...
            noise_arm = noise[..., :self.action_arm_dim].contiguous()
            noise_pred_arm = noise_pred[..., :self.action_arm_dim].contiguous()
            noise_hand = noise[..., self.action_arm_dim:].contiguous()
            noise_pred_hand = noise_pred[..., self.action_arm_dim:].contiguous()

            # L2 loss
            loss = F.mse_loss(noise_pred_arm, noise_arm) * 0.7 + F.mse_loss(noise_pred_hand, noise_hand) * 0.3
    ...
    def log_info(self, info):
        ...
        """
        log = super(DiffusionPolicyUNetDex, self).log_info(info)
