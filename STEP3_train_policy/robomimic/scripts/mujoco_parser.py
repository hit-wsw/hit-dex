...
class MuJoCoParserClass(object):
    ...
    def grab_rgbd_img_hw(self,height,width):
        """
            Grab RGB and Depth images
        """
    ...
    def get_pcd_from_depth_img1(self,depth_img,fovy=45):
        """
    ...
    def get_pcd_from_depth_img(self, depth_img, rgb_img, fovy=45):
        """
            Get colored point cloud data from depth image and RGB image.
        """
        # Get camera pose
        T_viewer = self.get_T_viewer()
        ...
        # Estimate 3D points from depth image
        xyz_img = meters2xyz(depth_img, cam_matrix)  # [H x W x 3]
        xyz_transpose = xyz_img.reshape(-1, 3).T  # [3 x N]
        xyzone_transpose = np.vstack((xyz_transpose, np.ones((1, xyz_transpose.shape[1]))))  # [4 x N]

        # Transform to world coordinates
        xyzone_world_transpose = T_viewer @ xyzone_transpose
        xyz_world_transpose = xyzone_world_transpose[:3, :]  # [3 x N]
        xyz_world = xyz_world_transpose.T  # [N x 3]

        # Normalize RGB values to [0,1]
        rgb_img_norm = rgb_img.astype(np.float32) / 255.0  # [H x W x 3]
        rgb_flatten = rgb_img_norm.reshape(-1, 3)  # [N x 3]

        # Concatenate (x, y, z, r, g, b)
        xyz_rgb_pcd = np.hstack((xyz_world, rgb_flatten))  # [N x 6]
        xyz_img_world = xyz_world.reshape(depth_img.shape[0],depth_img.shape[1],3)

        return xyz_rgb_pcd, xyz_img,xyz_img_world
    ...
    def get_egocentric_rgbd_pcd(
        ...
        if rsz_rate_for_pcd is not None:
            ...
            depth_img_rsz = depth_img; rgb_img_rsz = rgb_img
        ...
        pcd,xyz_img,xyz_img_world = self.get_pcd_from_depth_img(depth_img_rsz,rgb_img_rsz,fovy=fovy) # [N x 3]
    ...
    def get_pcd_from_rgbd_img(self,depth_img,rgb_img,fovy=45):
        """
            Get point cloud data from depth image
        """
        # Get camera pose
