...
from visualizer_sing import *

def on_press(key):
    global next_frame, delta_movement_accu, delta_ori_accu, adjust_movement, adjust_right, frame, step
    ...
    if os_type == "Linux":
        ...
        if key.char == '6':  # x negative
            ...
            if (delta_movement_accu != np.array([0.0, 0.0, 0.0])).any() or (delta_ori_accu != np.array([0.0, 0.0, 0.0])).any():
...
class ReplayDataVisualizer(DataVisualizer):
    ...
    def replay_keyframes_calibration(self):
        ...
        for cylinder in self.right_line_set:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--directory", type=str, default="save_data_scenario_1", help="Directory with saved data")
